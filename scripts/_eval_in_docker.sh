...
EVAL_INPUT_DIR="${EVAL_INPUT_DIR:-/workspace/data/benchmark/effi-sql}"
...
case "${BENCHMARK_SCALE}" in
  ...
  *)
    echo "[eval] Unsupported BENCHMARK_SCALE=${BENCHMARK_SCALE}. Use one of: default, scale"
    exit 1
    ;;
esac
...
case "${EVAL_SQL_MODE}" in
  ...
  *)
    echo "[eval] Unsupported EVAL_SQL_MODE=${EVAL_SQL_MODE}. Use one of: patch, end2end"
    exit 1
    ;;
esac
...
case "${EVAL_RESPONSE_FIELD}" in
  ...
  *)
    echo "[eval] Unsupported EVAL_RESPONSE_FIELD=${EVAL_RESPONSE_FIELD}. Use one of: prediction, raw_response"
    exit 1
    ;;
esac

if ! docker ps --format '{{.Names}}' | grep -q '^so_eval_env$'; then
  echo "[eval] so_eval_env is not running. Start sandbox first: bash scripts/db_up.sh"
  exit 1
fi
...
run_eval_for_dialect() {
  ...
  local output_dir="/workspace/outputs/${d}"
	  ...
	  case "${d}" in
	    postgres|postgresql) script="/workspace/diff_sql/evaluation/eval_postgresql_execution.py" ;;
	    *)
	      echo "[eval] Unsupported dialect in runner: ${d}"
	      return 1
      ;;
  esac
  ...
  echo "[eval] DIALECT=${d}"
  echo "[eval] BENCHMARK_SCALE=${BENCHMARK_SCALE}"
  echo "[eval] EVAL_SQL_MODE=${EVAL_SQL_MODE}"
  echo "[eval] EVAL_RESPONSE_FIELD=${EVAL_RESPONSE_FIELD}"
  echo "[eval] INPUT=${EVAL_INPUT_DIR}"
  if [[ -n "${EVAL_INPUT_FILE}" ]]; then
    echo "[eval] INPUT_FILE=${EVAL_INPUT_FILE}"
  fi
  echo "[eval] OUTPUT=${output_dir}"
  echo "[eval] Running in so_eval_env: ${cmd}"
  docker exec -it \
    -e EVAL_DIALECT="${d}" \
